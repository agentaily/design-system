...
.ax-datatable-box { border: 1px solid var(--border-default); border-radius: var(--radius-3); overflow: hidden; }
.ax-datatable-wrap { width: 100%; overflow: auto; }
.ax-datatable { width: 100%; border-collapse: separate; border-spacing: 0; font-size: var(--text-sm); }
.ax-datatable thead th { position: sticky; top: 0; z-index: 2; text-align: left; padding: 10px 14px; background: var(--surface-card); border-bottom: 1px solid var(--border-strong); white-space: nowrap; font-family: var(--font-mono); font-size: var(--text-xs); font-weight: var(--weight-medium); letter-spacing: var(--tracking-label); text-transform: uppercase; color: var(--text-faint); }
...
.ax-datatable__foot { display: flex; align-items: center; justify-content: space-between; padding: 10px 14px; border-top: 1px solid var(--border-default); font-family: var(--font-mono); font-size: var(--text-xs); color: var(--text-faint); background: var(--surface-card); }
`;
...
export function DataTable({ columns = [], rows = [], pageSize, caption, className = "", ...rest }) {
  ...
  return (
    <div className={className} {...rest}>
      <div className="ax-datatable-box">
          ...
          <table className="ax-datatable">
              ...
              <tr>
                {cols.map((c) => (
                  <th
                    key={c.key}
                    className={
                      (c.sortable !== false ? "th--sortable " : "") +
                      (c.numeric ? "ax-datatable__num" : "")
                    }
            ...
            <tbody>
              {paged.map((row, ri) => (
                <tr key={ri}>
                  {cols.map((c) => (
                    <td key={c.key} className={c.numeric ? "ax-datatable__num" : ""}>
