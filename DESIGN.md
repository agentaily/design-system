...
| `components/settings/` | **TestRow, HelpSteps, ConnectionCard, DeepSeekCard, FeishuCard, IntegrationSettings** (alias `ServiceConnections`) — connection-card primitives + the shared connection-card shell (built on `Card`) + two pure-display service cards + the composed service-connection section |
...
**The settings chain nests four layers, each built on the one below:** `PanelSheet` (floating shell: Header + optional Footer slot) → `SettingsSheet` (a floating settings page on PanelSheet: left section nav + right content) → `IntegrationSettings` / alias `ServiceConnections` (the 「集成」 section dropped into the content slot: hero + readiness rail) → connection **cards** composed into the section via `children` (`DeepSeekCard`, `FeishuCard`, …). `PanelFooter` is the optional **footer-content** component (status-left / actions-right) you pass to a sheet's `footer` slot. **`SettingsSaveBar`** is the per-tab **bottom save bar** (GitHub model — explicit commit, not auto-save): one bar at the bottom of each settings tab, gated on dirty, pairing with `Form.useForm` (or explicit `dirty`/`onSave` for non-form content like the connection section). Each layer is reusable on its own — `PanelSheet` for any full-screen panel, `SettingsSheet` for any floating settings page, `IntegrationSettings` for the integrations section, a single card for one connection. The left nav items (集成 / 通用 / 账户 …) are the consumer's to configure via `SettingsSheet`'s `nav` prop.
...
PanelSheet                         浮层外壳:Header + body + 可选 Footer 槽
  └ SettingsSheet                  设置页:左侧分区导航(nav) + 右侧内容 + footer 槽
      ├ 内容(按 active tab 切换):
      │   ├ 集成   IntegrationSettings ── (built on PageSection) hero + 就绪栏 + 连接卡槽(children)
      │   │            └ DeepSeekCard / FeishuCard ── 组合 ConnectionCard(← built on Card)
...
- **每个分区的「眉标 + 标题 + 描述 + 正文」用 `PageSection`** —— 通用页面分区布局（不限于设置），集成/通用/账户共用同一头部(`IntegrationSettings` 本身就基于它)。`SettingsSection` 作为别名保留。
...
| **L4 Page shells / frames** | full-page layout with slots | L1–L3 | `AppShell`, `DesignerShell`, `DocsLayout`, `SignInPage`, `PanelSheet` (floating shell) → `SettingsSheet` (settings page on it) → `IntegrationSettings` (a section in it) |
