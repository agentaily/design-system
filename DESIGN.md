...
| `components/layout/` | **AspectRatio, ScrollArea, Resizable, Sidebar, AppShell, DesignerShell, DocsLayout, PanelSheet, SettingsSheet, PageSection, PanelFooter, SettingsSaveBar** — incl. full-page shells/frames + the full-screen rise-up overlay shell, the floating settings page built on it, the per-section content layout (eyebrow/title/description + body), its optional footer-content component, and the per-tab save bar |
...
| `components/settings/` | **TestRow, HelpSteps, ConnectionCard, DeepSeekCard** — connection-card primitives + the shared connection-card shell (built on `Card`) + a pure-display service card |
| `components/auth/` | **AuthDialog (+ AuthDialog.useAuth), AccountControl, SignInPage, VerifyEmailPage (+ VerifyEmailPage.useVerify)** — sign-in/register modal + persisted session + top-bar account menu + full-page sign-in + full-page email verification |
| `components/review/` | **MarkupLayer** — point-at-an-element review overlay (`data-mk-label`) |
| **Hooks** (headless) | `Queue.useQueue`, `AuthDialog.useAuth`, `VerifyEmailPage.useVerify`, `Form.useForm` / `Form.useFieldArray` — logic without UI, exposed as statics on the paired component |
| `ui_kits/chat/` | Chat app (interactive) |
| `ui_kits/website/` | Marketing landing page |
| `ui_kits/docs/` | Documentation site (interactive) |
| `SKILL.md` | Agent-skill entry point |

Every component ships `<Name>.jsx` + `<Name>.d.ts` (props) + `<Name>.prompt.md` (usage) — **148 component exports** across the primitive categories (buttons, inputs, display, feedback, overlay, layout, chat, ai, code, voice, workflow, utilities) plus product-domain layers (**settings, auth, review**). Full-page frames — `AppShell` / `DesignerShell` / `DocsLayout` / `PanelSheet` / `SettingsSheet` (layout), `ConversationThread` (chat), `SignInPage` (auth) — are **live components, not copy-templates**: change one, every consuming project benefits on re-sync. Consume via the compiled bundle: `window.AxiomDesignSystem_7fc962`. Read each component's `.prompt.md` for copy-paste usage.

**Two overlay shells, do not confuse them:** `Sheet` (overlay/) is an **edge drawer** (slides in from right/left/bottom at ~420px over a blurred scrim); `PanelSheet` (layout/) is a **full-screen rise-up overlay** (covers the viewport, fades the scrim, rises the panel, bar + scroll body + optional sticky footer). `PanelSheet` is the single source of the overlay-shell CSS (`.ax-psheet*`) — downstream code that copied a `.s-overlay/.s-modal/.s-wrap` shell should delete it and mount `<PanelSheet>`.

**The settings chain nests layers, each built on the one below:** `PanelSheet` (floating shell: Header + optional Footer slot) → `SettingsSheet` (a floating settings page on PanelSheet: left section nav + right content) → a 「集成」 section you compose (e.g. a `PageSection` hero) → connection **cards** dropped into the section via `children` (`DeepSeekCard`, …). `PanelFooter` is the optional **footer-content** component (status-left / actions-right) you pass to a sheet's `footer` slot. **`SettingsSaveBar`** is the per-tab **bottom save bar** (GitHub model — explicit commit, not auto-save): one bar at the bottom of each settings tab, gated on dirty, pairing with `Form.useForm` (or explicit `dirty`/`onSave` for non-form content like the connection section). Each layer is reusable on its own — `PanelSheet` for any full-screen panel, `SettingsSheet` for any floating settings page, a single card for one connection. The left nav items (集成 / 通用 / 账户 …) are the consumer's to configure via `SettingsSheet`'s `nav` prop.
...
- **集成分区不基于 `Form.useForm`.** 连接卡有掩码密钥(留空=不变)+ 独立的「测试连接」动作,所以用**各卡片自己的受控状态**,通过**显式 dirty**(`cfg` vs `savedCfg`)接 `SettingsSaveBar`(传 `dirty`/`saving`/`onSave`/`onReset`)。
...
- **保存粒度 = 每个 tab 一条底栏**(GitHub 模型,显式提交)。`footer` 按 `active` 计算,无需保存的 tab 传 `footer={null}`。
...
| **L3 Patterns** (organisms) | self-contained interaction + state, configurable | L1–L2 | `AuthDialog`, `MarkupLayer`, `ConversationThread`, `DeepSeekCard` (display-only — caller owns state) |
| **L4 Page shells / frames** | full-page layout with slots | L1–L3 | `AppShell`, `DesignerShell`, `DocsLayout`, `SignInPage`, `VerifyEmailPage`, `PanelSheet` (floating shell) → `SettingsSheet` (settings page on it) |
...
`buttons · inputs · display · feedback · overlay · layout · chat · ai · code · voice · workflow · utilities · settings · auth · review`. A product-domain component (e.g. `DeepSeekCard`) lives at its **abstraction layer** and is merely *filed* under its **domain folder** (`settings/`). Domains are folders, not tiers.
...
For internal unification, push anything shared **up into a component** (live, single-source) — not into a template. Templates are one-off scaffolds meant to be forked and to diverge. This is exactly why the designer shell and the former `starters/` pages now live as **components** (`AppShell`, `DesignerShell`, `DocsLayout`, `SettingsSheet`, `ConversationThread`, `SignInPage`): fix the shell once and every project benefits on re-sync. The single rule that keeps layering from rotting: **dependencies are strictly one-way down — a lower layer never reaches up.**
...
| `VerifyEmailPage.useVerify({ verifyToken })` | `{ status, error, retry }` — verifying → ok / error flow runner (throws/network → error) | `<VerifyEmailPage>` |
