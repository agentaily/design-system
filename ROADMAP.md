...
- **locale-agnostic 文案(headless 派)** — DS **不自带 i18n**:用户可见文案走 `copy` prop(**英文默认 fallback** + 消费方覆盖),沿用 `SignInPage` / `AuthDialog` 的 `DEFAULT_COPY` idiom。settings/账户区已 headless 化:`DeepSeekCard`(`copy` 一处贯穿 `ConnectionCard` → `TestRow`)、`ConnectionCard`(`copy` + `testCopy`)、`TestRow`、`SettingsSaveBar`、`AccountControl` 各带 `copy`;`SettingsSheet` 的 `crumb` / `navLabel` 默认翻英文。`HelpSteps` / `PageSection` 本就 headless。下游中文产品需传 `L()` copy。(#35;`StatusPill` badge 文案 follow-up)
